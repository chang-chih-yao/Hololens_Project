...
class GroupNormalize(object):
    def __init__(self, mean, std):
        self.mean = mean      # a list
        self.std = std        # a list

    def __call__(self, tensor):
        rep_mean = self.mean * (tensor.size()[0]//len(self.mean))     # list 乘以一個數字會複製一份, arr = [1, 2, 3], arr*2 = [1, 2, 3, 1, 2, 3]
...
class Stack(object):
    ...
    def __call__(self, img_group):
        # r, g, b = img_group[0].getpixel((0, 0))
        # print(r, g, b)
        if img_group[0].mode == 'L':         # gray
...
if __name__ == "__main__":
    trans = torchvision.transforms.Compose([
        GroupMultiScaleCrop(256, [1, .97, .95]),
        Stack(roll=False),
